...
import { walletTransferInterval } from "../settings";
import { sleep } from "./util";
import { initWaiting, logToFile } from "./msgLog";
import { COMPUTE_UNIT_PRICE, connection } from "../config";
    ...
    try {
        ...
        try {
            await Promise.all(txs.map(async (transaction, i) => {
                const randomInterval = (Math.random() + 1) * walletTransferInterval / 2
                await sleep(1000 * randomInterval)
...
const handleTxs = async (transaction: Transaction, mainKp: Keypair, index: number) => {
    try {
        ...
        if (!sig) {
            const sig = await sendAndConfirmTransaction(connection, transaction, [mainKp], { skipPreflight: true })
            if (sig) logToFile(`Success in distribution from bundler ${index + 1} => https://solscan.io/tx/${sig}`);
            else {
                const sig = await sendAndConfirmTransaction(connection, transaction, [mainKp], { skipPreflight: true })
                if (sig) logToFile(`Success in distribution from bundler ${index + 1} => https://solscan.io/tx/${sig}`);
            }
        } else {
            logToFile(`Success in distribution from bundler ${index + 1} => https://solscan.io/tx/${sig}`);
        }
    } catch (error) {
        logToFile(`Error in distribution from bundler ${index + 1} => : ${error}`)
        initWaiting()
    }
}